...
class IncomingCallController extends Controller
{
    ...
    public function handleAvailableNumberCall($to)
    {
        ...
        foreach ($relevantBids as $bid) {
            ...
            $twimlBody .= '<Dial callerId="+441156471655" timeout="20"><Client statusCallbackMethod="GET" statusCallbackEvent="initiated ringing failed no-answer" statusCallback="https://allcalls.io/api/handle-call-status?user_id=' . $user_id . '&amp;call_type_id=' . $call_type_id . '&amp;from=' . $availableNumber->from . '">' . $user_id . '</Client></Dial>';
        }
