...
class IncomingCallController extends Controller
{
    ...
    public function handleAvailableNumberCall($to)
    {
        ...
        if (!empty($relevantBids)) {
            ...
            $twimlBody = '<Dial callerId="+441156471655" action="https://allcalls.io/api/handle-call-status?user_id=' . $user_id . '&call_type_id=' . $call_type_id . '&from=' . $availableNumber->from . '" timeout="20">' . $user_id . '</Dial>';
        }
