...
class IncomingCallController extends Controller
{
    ...
    public function handleAvailableNumberCall($to)
    {
        ...
        foreach ($relevantBids as $bid) {
            ...
            $twimlBody .= '<Dial "https://allcalls.io/api/handle-call-status?user_id=' . $user_id . '&call_type_id=' . $call_type_id . '&from=' . $availableNumber->from . '" callerId="+441156471655" timeout="20"><Client>' . $user_id . '</Client></Dial>';
        }
