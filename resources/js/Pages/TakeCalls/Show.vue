...
let setOnlineCallType = () => {
    for ( let i = 0; i < callTypesWithToggles.value.length; i++ ) {
        if (callTypesWithToggles.value[i].callType.id === props.onlineCallType.call_type_id) {
...
let callTypesWithToggles = ref(
  props.callTypes.filter(callType => callType.selected).map((callType) => {
    ...
    <section class="p-3">
      <div class="mx-auto max-w-screen-xl sm:px-12">
        ...
        <ul class="max-w-md divide-y divide-gray-200 mb-8">
          ...
          >
            <!-- Title on the left -->
            <p class="text-xl text-gray-900" v-text="callType.callType.type"></p>

            <!-- Toggle on the right -->
            <label class="relative inline-flex items-center cursor-pointer">
              <input type="checkbox" v-model="callType.toggle" class="sr-only peer" @change="toggled($event, callType.callType)" />
              <div
