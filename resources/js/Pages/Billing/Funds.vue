    ...
    <div class="sm:py-10">
      <form class="mx-auto max-w-7xl">
          ...
          <section class="mx-auto sm:px-6 lg:px-8 space-y-6">
              <div class="p-4 sm:p-8 sm:rounded-lg">
                ...
                <div :class="{
                  ...
                  'border border-custom-indigo bg-custom-blue text-blue-600 shadow-2xl font-bold': selectedCardId === card.id,
                  'border border-gray-500 text-gray-500': selectedCardId !== card.id,
                }" v-for="card in cards" :key="card.id" @click.prevent="selectCard(card.id)">
                  <div class="ml-2 text-sm font-medium flex items-center">
                    <span class="uppercase mr-4">{{ card.type }}</span> **** **** **** {{ card.last4 }}
                  </div>
                </div>
                <div class="mt-4 max-w-lg">
                  <label for="amount" class="block mb-2 text-sm font-medium text-custom-white">Amount</label>
                  <div class="relative mb-6">
                    <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none text-custom-white">
                      $
                    </div>
                    <TextInput type="number" id="amount" class="w-full"
                      placeholder="500" v-model="amount" :disabled="selectedCardId === 0" />
                  </div>
                </div>
                <AuthenticatedButton type="button" class="" @click.prevent="addFunds">
                  Add funds
                </AuthenticatedButton>
              </div>
          </section>
        ...
        <div v-else class="text-center sm:px-6 lg:px-8">
          <p class="text-gray-300 mt-6 mb-8">You haven't added any cards yet.</p>
