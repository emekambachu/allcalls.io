...
watch(() => props.showActivitiesModal, async (newValue) => {
  if (newValue) {
    // Call APIs here and update the form data accordingly
    await Fetehactivities(props.user.id);
  }
});
                  ...
                  <table class="w-full text-sm text-left text-gray-400">
                    <thead class="text-xs text-gray-300 uppercase bg-sky-900">
                      <tr>
                        ...
                        <th scope="col" class="px-4 py-3">Usage Time</th>
                    ...
                    <tbody>
                      <tr v-for="activity in activities.data" :key="activity.id" class="border-b border-gray-500">
                        ...
                        <td class="text-gray-600 px-4 py-3">{{ activity.logout_time }}</td>
                        <td class="text-gray-600 px-4 py-3">{{ activity.last_activity_at }}</td>
                        <td class="text-gray-600 px-4 py-3">{{ activity.usage_time }}</td>
                        <td class="text-gray-600 px-4 py-3">{{ activity.session_id }}</td>
