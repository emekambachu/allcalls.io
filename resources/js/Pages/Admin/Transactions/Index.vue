...
let paginate = (url) => {
    ...
    if (transactionType.value && transactionType.value !== 'Choose transaction Type' || paymentSubmitedBy.value && paymentSubmitedBy.value !== 'Payment Submited By' || form.value.transaction_date) {
        router.visit(url, {
            data: {
                "transaction_type": transactionType.value !== 'Choose transaction Type' ? transactionType.value : null,
                "submited_by": paymentSubmitedBy.value !== 'Payment Submited By' ? paymentSubmitedBy.value : null,
        ...
        <div class="px-3">
            <div class="mx-auto max-w-screen-xl sm:px-12">
                <div class="mb-4 grid xl:grid-cols-4 lg:grid-cols-3 mb-2 md:grid-cols-2 sm:grid-cols-1 gap-4">
                    ...
                    <VueDatePicker v-model="dateRange" placeholder="Select Date Range" formatRange range />

                    <div>
                        <PrimaryButton type="button" class="ml-2" @click.prevent="fetchData">
                            <global-spinner :spinner="isLoading" /> Filter
                        </PrimaryButton>
                        <button @click.prevent="ResetPage()" type="button"
                                ...
                                <tr v-for="transaction in transactions.data" :key="transaction.id"
                                    ...
                                    <td class="text-gray-600 px-4 py-3">
                                        ...
                                        <span title="Visa" v-if="detectCardType(transaction.card?.card_number) == 'Visa'">
                                            <svg class="w-16 h-16" viewBox="0 -140 780 780"
                                        ...
                                        <span title="American Express"
                                            v-if="detectCardType(transaction.card?.card_number) == 'American Express'">
                                            <svg class="w-16 h-16 " viewBox="0 -9 58 58" fill="none"
                                                xmlns="http://www.w3.org/2000/svg">
                                        ...
                                        <span title="Diners Club"
                                            v-if="detectCardType(transaction.card?.card_number) == 'Diners Club'">
                                            <svg class="w-16 h-16 " version="1.1" id="Layer_1"
                                                xmlns="http://www.w3.org/2000/svg"
                                        ...
                                        <span title="UnionPay"
                                            ...
                                            <svg class="w-16 h-16 " enable-background="new 0 0 780 500"
