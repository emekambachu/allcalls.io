...
if(requestData.first_six_card_no){
    form.value.first_six_card_no = requestData.first_six_card_no
  }
  if(requestData.last_four_card_no){
    form.value.last_four_card_no = requestData.last_four_card_no
  }
...
let fetchData = () => {
  ...
  firstStepErrors.value.first_six_card_no = ['']
    ...
    if (form.value.first_six_card_no) {
  ...
  router.visit(`/admin/transactions`, {
    data: {
      ...
      first_six_card_no: form.value.first_six_card_no ?  form.value.first_six_card_no.toString() : null,
      last_four_card_no:form.value.last_four_card_no ? form.value.last_four_card_no.toString() : null,
...
let firstSixDigit = (e) => {
    if (e.target.value.length > 6) {
        ...
        form.value.first_six_card_no = e.target.value
    }
}
let lastFourDigit = (e) => {
    if (e.target.value.length > 4) {
        e.target.value = e.target.value.slice(0, 4);
        form.value.last_four_card_no = e.target.value
    }
}
    ...
    <div class="px-3">
      <div class="mx-auto max-w-screen-xl sm:px-12">
        <div class="mb-4 grid xl:grid-cols-4 lg:grid-cols-3 mb-2 md:grid-cols-2 sm:grid-cols-1 gap-4">
                  ...
                  <li class="w-full">
                    <div class="flex items-center ps-3">
                      ...
                      <label for="horizontal-list-radio-phone"
                        class="w-full py-3 ms-2 text-sm font-medium text-gray-900">Filter by phone number</label>
                    </div>
                  </li>
          ...
          <div>
            <input type="text" id="first_six_card_no" v-model="form.first_six_card_no" @input="firstSixDigit" required placeholder="First 6 digit  of  card number" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"  >
            <div v-if="firstStepErrors.first_six_card_no" class="text-red-500" v-text="firstStepErrors.first_six_card_no[0]"></div>
          </div>
          <div>
            <input type="text" id="last_four_card_no" v-model="form.last_four_card_no" @input="lastFourDigit" required placeholder="Last 4 digit of  card number" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"  >
            <div v-if="firstStepErrors.last_four_card_no" class="text-red-500" v-text="firstStepErrors.last_four_card_no[0]"></div>
          </div>

          <VueDatePicker v-model="dateRange" placeholder="Select Date Range" auto-apply :format="formatRange" range />
                ...
                <tr v-for="transaction in transactions.data" :key="transaction.id" class="border-b border-gray-500">
                  ...
                  <td class="text-gray-600 px-4 py-3">
                    <span title="Visa" v-if="detectCardType(transaction.card?.card_number) == 'Visa'">
                      <svg class="w-16 h-16" viewBox="0 -140 780 780" enable-background="new 0 0 780 500" version="1.1"
                    ...
                    <span title="Mastercard" v-if="detectCardType(transaction.card?.card_number) == 'Mastercard'">
                      <svg class="w-16 h-16" viewBox="0 -140 780 780" enable-background="new 0 0 780 500" version="1.1"
                        xml:space="preserve" xmlns="http://www.w3.org/2000/svg">
                        <path
                          d="M40,0h700c22.092,0,40,17.909,40,40v420c0,22.092-17.908,40-40,40H40c-22.091,0-40-17.908-40-40V40   C0,17.909,17.909,0,40,0z"
                    ...
                    <span title="American Express" v-if="detectCardType(transaction.card?.card_numberzzz) ==
                      'American Express'
                      ">
                      <svg class="w-16 h-16" viewBox="0 -9 58 58" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <span title="Diners Club" v-if="detectCardType(transaction.card?.card_number) == 'Diners Club'
                      ">
                    ...
                    <span title="UnionPay" v-if="detectCardType(transaction.card?.card_number) == 'UnionPay'">
                      <svg class="w-16 h-16" enable-background="new 0 0 780 500" viewBox="0 0 780 500"
                    ...
                    <span title="Discover" v-if="detectCardType(transaction.card?.card_number) == 'Discover'">
                      <svg class="w-16 h-16" viewBox="0 -140 780 780" xmlns="http://www.w3.org/2000/svg">
                        <g fill-rule="evenodd">
                          <path
                    ...
                    <span title="JCB" v-if="detectCardType(transaction.card?.card_number) == 'JCB'">
                      <svg class="w-16 h-16" viewBox="0 -140 780 780" enable-background="new 0 0 780 500" version="1.1"
                        xml:space="preserve" xmlns="http://www.w3.org/2000/svg">
                        <path
                          d="M40,0h700c22.092,0,40,17.909,40,40v420c0,22.092-17.908,40-40,40H40c-22.091,0-40-17.908-40-40V40   C0,17.909,17.909,0,40,0z"
                        ...
                        <linearGradient id="c" x1="83.977" x2="84.977" y1="645.5" y2="645.5"
                          gradientTransform="matrix(132.87 0 0 -323.02 -10686 2.0876e5)" gradientUnits="userSpaceOnUse">
                        ...
                        <linearGradient id="b" x1="83.984" x2="84.979" y1="645.5" y2="645.5"
                          gradientTransform="matrix(133.43 0 0 -323.02 -11031 2.0876e5)" gradientUnits="userSpaceOnUse">
                        ...
                        <linearGradient id="a" x1="83.978" x2="84.977" y1="645.5" y2="645.5"
                          gradientTransform="matrix(132.96 0 0 -323.03 -10842 2.0877e5)" gradientUnits="userSpaceOnUse">
